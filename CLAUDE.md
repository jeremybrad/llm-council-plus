...
`timeout_seconds` is live. `max_parallel` is capped at 2 when any `agentcli:` seat is in the run. Predicted calls above `roundtable_max_calls_per_run` fail before the first invocation.
...
| `agentcli:codex` | `codex-subscription` | `login status` equals `Logged in using ChatGPT` | `exec --json --sandbox read-only --ephemeral --skip-git-repo-check -` (stdin). Uses the **installed CLI default model**; no unverified `-m` pin |
